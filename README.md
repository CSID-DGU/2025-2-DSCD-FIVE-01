...
## 3) VectorDB 구축: 데이터 수집 → 정제 → 인덱싱 → 검색 (상세)
...
### 3.1 사용 데이터(텍스트 전용)
> 실제 경로/파일명은 프로젝트에 맞춰 수정하세요. 아래는 권장 스키마 예시입니다.
...
> 모든 본문은 **문단 단위**로 관리하고, 문단 길이는 300~1200자 범위를 권장합니다(너무 짧으면 의미가 약하고, 너무 길면 검색 정밀도가 낮아짐).
